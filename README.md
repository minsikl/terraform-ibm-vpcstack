...
| `boot_volume_encryption_mode` | Boot volume encryption: `"default"` (IBM provider-managed key, default) or `"byok"` (customer-managed via Key Protect / lab4193-kms) | `string` | `"default"` | no |
...
- `boot_volume_encryption_mode` accepts two values:
  ...
  - `"byok"` — customer-managed key via Key Protect; requires `lab4193-kms` instance and `lab4193-boot-key` root key to exist in the account (provisioned by `ibmcloud-setup/`), and an IAM authorization policy allowing VPC Block Storage (`server-protect`) to use the key
